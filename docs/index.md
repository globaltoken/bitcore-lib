...
To get started, just `npm install gltcore` or `bower install gltcore`.
...
var imported = gltcore.PrivateKey.fromWIF(exported);
...
var p2shAddress = new gltcore.Address([publicKey1, publicKey2, publicKey3], 2);
```
...
var uri = new gltcore.URI(paymentInfo).toString();
```
