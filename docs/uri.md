...
## URI Validation
The main use that we expect you'll have for the `URI` class in gltcore is validating and parsing bitcoin URIs. A `URI` instance exposes the address as a gltcore `Address` object and the amount in Satoshis, if present.
